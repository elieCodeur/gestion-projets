...
import { Loader2, KanbanSquare, Calendar, Flag, MessageSquare, Edit, Trash2, PlusCircle } from 'lucide-react';
...
// Schéma de validation pour l'édition de projet
...
// Schéma de validation pour la création/édition de sprint
...
const ProjectDetailPage: React.FC = () => {
  ...
  const [currentSprint, setCurrentSprint] = useState<Sprint | null>(null); // Pour l'édition de sprint
  ...
  const handleEditProject = (data: EditProjectFormInputs) => {
    ...
    api.put(`/projects/${project.id}`, data)
      ...
      .catch((err) => {
        setServerError(err.response?.data?.message || "Une erreur s'est produite lors de la modification du projet.");
      });
  };
  ...
  const handleDeleteProject = () => {
    ...
    api.delete(`/projects/${project.id}`)
      ...
      .catch((err) => {
        setServerError(err.response?.data?.message || "Une erreur s'est produite lors de la suppression du projet.");
      });
  };
  ...
  const handleCreateSprint = (data: SprintFormInputs) => {
    ...
    api.post('/sprints', { ...data, projectId: Number(projectId) })
      ...
      .catch((err) => {
        setServerError(err.response?.data?.message || "Une erreur s'est produite lors de la création du sprint.");
      });
  };
  ...
  const handleUpdateSprint = (data: SprintFormInputs) => {
    ...
    api.put(`/sprints/${currentSprint.id}`, data)
      ...
      .catch((err) => {
        setServerError(err.response?.data?.message || "Une erreur s'est produite lors de la modification du sprint.");
      });
  };
  ...
  const handleDeleteSprint = (sprintId: number, sprintName: string) => {
    ...
    api.delete(`/sprints/${sprintId}`)
      ...
      .catch((err) => {
        setServerError(err.response?.data?.message || "Une erreur s'est produite lors de la suppression du sprint.");
      });
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-full">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex space-x-3">
            <Link to={`/projects/${project.id}/messages`}>
              <button className="flex items-center bg-green-500 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-green-600 transition duration-300">
                <MessageSquare className="mr-2 h-5 w-5" /> Messagerie
              </button>
            </Link>
            <button
              onClick={openEditProjectModal}
              className="flex items-center bg-blue-500 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-blue-600 transition duration-300"
            >
              <Edit className="mr-2 h-5 w-5" /> Modifier
            </button>
            <button
              onClick={handleDeleteProject}
              className="flex items-center bg-red-500 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-red-600 transition duration-300"
            >
              ...
              <div key={sprint.id} className="bg-white p-4 border rounded-lg shadow-sm hover:shadow-md transition">
                <div className="flex flex-col sm:flex-row justify-between items-start">
                  <div>
                    ...
                    <span className={`text-xs font-semibold px-2 py-1 rounded-full ${getStatusClasses(sprint.status)}`}>
                      <Flag className="inline-block mr-1 h-3 w-3" />
                      {sprint.status}
                    </span>
                  </div>
                ...
                <div className="mt-4 flex justify-end space-x-2">
                  <button
                    ...
                    className="flex items-center bg-blue-500 text-white font-semibold py-2 px-3 rounded-lg shadow-md hover:bg-blue-600 transition duration-300"
                  >
                    <Edit className="mr-2 h-5 w-5" /> Modifier
                  </button>
                  <button
                  ...
                  <Link to={`/projects/${project.id}/sprints/${sprint.id}/kanban`}>
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
            ...
            <form onSubmit={handleProjectSubmit(handleEditProject)} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 ${
                    projectErrors.name ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-indigo-500'
                  }`}
                />
              ...
              <div className="flex justify-end gap-3">
                ...
                >
                  {projectIsSubmitting ? 'Modification...' : 'Enregistrer'}
                </button>
              </div>
            </form>
          </div>
      ...
      {/* Modal de création/édition de sprint */}
      {showSprintModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
            <h2 className="text-xl font-bold mb-4 text-gray-800">
              {currentSprint ? 'Modifier le Sprint' : 'Ajouter un nouveau Sprint'}
            </h2>
            <form onSubmit={handleSprintSubmit(currentSprint ? handleUpdateSprint : handleCreateSprint)} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  {...registerSprint('name')}
                  className={`w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 ${
                    sprintErrors.name ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-indigo-500'
                  }`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 ${
                    sprintErrors.startDate ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-indigo-500'
                  }`}
                />
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 ${
                    sprintErrors.endDate ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-indigo-500'
                  }`}
                />
                {sprintErrors.endDate && <p className="mt-1 text-sm text-red-600">{sprintErrors.endDate.message}</p>}
              </div>
              {currentSprint && ( // Le statut n'est modifiable que lors de l'édition
                <div>
                  <label htmlFor="sprintStatus" className="block mb-2 font-medium text-gray-700">Statut</label>
                  <select
                    id="sprintStatus"
                    {...registerSprint('status')}
                    className="w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 border-gray-300 focus:ring-indigo-500"
                  >
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  disabled={sprintIsSubmitting}
                  className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition duration-300"
                >
                  {sprintIsSubmitting ? (currentSprint ? 'Modification...' : 'Création...') : (currentSprint ? 'Enregistrer' : 'Créer')}
                </button>
              </div>
            </form>
          </div>
